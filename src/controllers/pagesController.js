...
module.exports = {
  async storePageConfig(req, res) {
    const { id } = req.params;
    const { firsPartOpt, detailsOpt } = req.body;

    try {
      await Products.findOneAndUpdate(
        { _id: id },
        { $set: { firsPartOpt, detailsOpt } }
      );
      return res
        .status(200)
  ...
  async saveCards(req, res) {
    ...
    const { header, color, description } = req.body;

    try {
      await Products.findOneAndUpdate(
        { _id: id },
        { $push: { cards: { header, color, description, image: filename } } }
      );
